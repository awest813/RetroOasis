...
- **Status:** core overhaul complete (Phases 1, 3, 4, 5a–e). Full suite 2717 pass; only 2 pre-existing `ui.test.ts` tab failures remain.
- **Optional follow-ups:** real-browser visual QA pass; onboarding/loading-overlay "CRT boot" copy; per-system arcade-cabinet accents; WCAG contrast spot-check on neon-on-black body text.
