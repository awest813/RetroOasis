...
## 2024-09-02 - Mitigate DOM-based XSS by removing innerHTML
**Vulnerability:** Use of `innerHTML` in `player.html`'s `fail()` and `showLoading()` functions introduced potential DOM-based XSS vulnerabilities and unsafe HTML string interpolation.
**Learning:** In unbundled, static HTML pages, passing strings into `innerHTML` poses an XSS risk. To preserve formatting like inline `<code>` tags while mitigating XSS, native DOM node creation is necessary. However, updating DOM elements directly can be cumbersome for nested structures.
**Prevention:** Construct HTML natively and use `DocumentFragment` objects to build complex elements safely without HTML string injection contexts. `DocumentFragment`s can be passed to UI rendering functions or attached to `Error` objects (e.g., `err.domMessage`) to render formatted messages safely.
